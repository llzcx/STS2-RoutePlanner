...
- 🧠 **DP 路径搜索** — 深度优先动态规划，毫秒级从**当前位置**找到通往 Boss 的最优路线，不仅限于地图起点，中途随时可用
...
- 🎨 **原生地图绘制** — 直接调用游戏内置画笔 API，沿地图路径圆点绘制平滑曲线，非叠加层 hack。联机模式下所有玩家互相可见
- 🗺️ **双 Boss 支持** — 同时处理主 Boss 和第二 Boss 的地图节点，全面覆盖
- ⚡ **黄金路径快速通道** — 检测并优化线性黄金路径走廊，返回即时结果
- 🌐 **国际化支持** — 完整中英文切换，易于扩展
